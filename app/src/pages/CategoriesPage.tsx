...
export default function CategoriesPage() {
  ...
  async function handleCreate() {
    ...
    const result = await createCategory({ userId: user.id, name: newName, type: newType, color: newColor })
    setSaving(false)
    if (!result) { setNewError('Erro ao criar categoria'); return }
    setNewOpen(false); setNewName(''); load()
  }
  ...
  const groups = (['expense', 'income', 'transfer'] as CategoryType[]).map(type => ({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
        ...
        {!loading && groups.map(g => (
          <div key={g.type}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {g.items.map(cat => (
                <div key={cat.id} className="card" style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
                  <CategoryIcon name={cat.icon} color={cat.color} size={36} />
                  <span style={{ flex: 1, fontSize: 14, fontWeight: 600, color: 'var(--color-text)' }}>{cat.name}</span>
                  {!cat.is_default && (
                    <button
      ...
      {/* New category modal */}
      <Modal open={newOpen} onClose={() => setNewOpen(false)} title="Nova Categoria" size="sm">
          ...
          <div className="input-wrapper">
            ...
            <div className="type-selector">
              {(['expense', 'income'] as CategoryType[]).map(t => (
                <button key={t} type="button" className={`type-selector-btn ${newType === t ? 'active' : ''}`} onClick={() => setNewType(t)}>
                  {TYPE_LABELS[t]}
                </button>
              ))}
            </div>
